...
public class FilteringStepDefs {
    ...
    public void select_all_boxes() {
        the_user_click_the_filter_button();
        the_user_click_on_manage_filter_button();

        BrowserUtils.waitFor(3);
        List<WebElement> checkBoxes = new FleetVehiclesPage().checkBoxes;
    ...
    public void user_can_remove_all_filterings_by_using_the_reset_button() {
        BrowserUtils.waitFor(3);
        Assert.assertFalse(new FleetVehiclesPage().firstElement.isDisplayed());
    ...
    public void user_can_apply_filtering_for_each_of_the_on_the_page(String dataColumns) {
        ...
        if (dataColumns.equals("Driver")){
            new FleetVehiclesPage().filterSearchBox.sendKeys(dataColumns);
            ...
            String actualDriverName = new FleetVehiclesPage().actualDriverName.getText();
